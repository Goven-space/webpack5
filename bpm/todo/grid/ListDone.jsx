...
import OpenProcessForm from '../form/OpenProcessForm';
...
const LIST_URL=URI.BPM.TODO.todoDoneList;
const DELETE_URL=URI.BPM.TODO.tododelete;
const CANCEL_URL=URI.BPM.ENGINE.cancelUrl;
...
class ListDone extends React.Component {
  constructor(props) {
    ...
    this.url=LIST_URL+"?applicationId="+this.applicationId; //应用id
    this.deleteUrl=DELETE_URL+"?applicationId="+this.applicationId; //应用id
    this.cancelUrl=CANCEL_URL+"?applicationId="+this.applicationId; //应用id
    this.state={
  ...
  cancel=()=>{
    if(this.state.selectedRowKeys.length>1){AjaxUtils.showError("一次只能撤回一个流程!");return;}
    this.setState({loading:true});
    let ids=this.state.selectedRowKeys.join(",");
    let postdata={applicationId:this.applicationId,processId:this.state.selectedRows[0].processId,transactionId:ids,userId:this.userId}
    AjaxUtils.post(this.cancelUrl,postdata,(data)=>{
  ...
  openForm=(record,userId)=>{
    this.setState({visible: true,currentRecord:record,approveUserId:userId});
  }
    ...
    const columns=[
      {
        ...
        ellipsis: true,
          ...
          title: '审批人(点击审批)',
          dataIndex: 'wf_authorUserIds',
          width: '15%',
          render: (text,record) => {
            return text.map((userId)=>{
                return <Tag color='blue' style={{cursor:'pointer'}} onClick={this.openForm.bind(this,record,userId)}>{userId}</Tag>
            });
          }
      },{
      title: '申请人',
    ...
    return (
      <div>
          <Modal key={Math.random()} title='' maskClosable={false}
              ...
              <OpenProcessForm record={this.state.currentRecord} approveUserId={this.state.approveUserId} processId={this.state.currentRecord.processId} transactionId={this.state.currentRecord.transactionId} close={this.closeModal} />
          </Modal>
          <Row style={{marginBottom:5}} gutter={0} >
            <Col span={10} >
            <ButtonGroup>
              <Button  type="ghost" onClick={this.showConfirm} icon="delete"  disabled={!hasSelected} >删除</Button>
              <Button  type="ghost" onClick={AjaxUtils.showConfirm.bind(this,'撤回流程','撤回流程重新提交',this.cancel)} icon="rollback"  disabled={!hasSelected}  >撤回</Button>
