...
const LISTURL=URI.BPM.APPLICATION.listapps;
const appUrl=URI.rootPath+"/bpm/application";
const { Meta } = Card;
		          ...
		          renderItem={item => {
								if(item.appId==='new'){
									//新建应用
										return (
										<List.Item>
											<Card hoverable={true} style={{padding:'3px 0 3px 0',height:'105px',backgroundColor:item.backgroundColor||'#f0f5ff'}} onClick={this.openApp.bind(this,item.appId)}  >
											<Meta
									...
									//普通应用
											return (
					            <List.Item>
					              <Card hoverable={true} size='small' style={{padding:'10px',overflow:'hidden',height:'105px',backgroundColor:item.backgroundColor||'#fff'}} onClick={this.openApp.bind(this,item.applicationId)}  >
					              <Meta
					                 avatar={<Badge count={item.processCount} showZero overflowCount={999}  style={{ backgroundColor: item.canRunFlag?'green':'#666666' }} ><Avatar size={64}  src={item.src||defaultImg} /></Badge>}
					                 title={<b>{item.applicationName}</b>}
					                 description={<span>
