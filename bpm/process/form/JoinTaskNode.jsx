...
import AjaxEditSelect from '../../../core/components/AjaxSelect';
...
class form extends React.Component{
  constructor(props){
    ...
    this.selectNodeUrl=SelectNodeUrl+"?processId="+this.processId;
    this.state={
      mask:false,
      formData:{},
    };
  ...
  loadNodePropsData=()=>{
        ...
        AjaxUtils.get(url,(data)=>{
            ...
            if(data.state===false){
              ...
              if(data.joinNodeId){data.joinNodeId=data.joinNodeId.split(",");}
        ...
        <Tabs size="large">
          <TabPane  tab="基本属性" key="props"  >
            ...
            <FormItem label="传入用户" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}  help="是否允许前继审批节点选择本节点的用户并传入" >
              {getFieldDecorator('allowSelectUser',{initialValue:'0'})
              (
                <RadioGroup>
            ...
            <FormItem label="聚合选项" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}>
              {getFieldDecorator('joinType',{initialValue:'1'})
              (
                <RadioGroup>
                  <Radio value='0'>等待所有节点结束后启动</Radio>
                  <Radio value='1'>等待指定节点结束后启动</Radio>
                </RadioGroup>
              )}
            </FormItem>
            <FormItem
              ...
              labelCol={{ span: 4 }}
              wrapperCol={{ span: 16 }}
              style={{display:this.props.form.getFieldValue("joinType")=='1'?"":"none"}}
              help='只有选中的节点全部结束后本节点才能启动'
            >
              {
                getFieldDecorator('joinNodeId', {
                  rules: [{ required: false}]
                })
                (<AjaxEditSelect url={this.selectNodeUrl} valueId='nodeId' textId='nodeName' options={{showSearch:true,mode:'tags'}} />)
              }
            </FormItem>
            <FormItem
              label="阶段"
