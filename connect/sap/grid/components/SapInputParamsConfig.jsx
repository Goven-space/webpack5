...
const SubmitUrl=URI.CONNECT.SAPRFC.inputparams;
const readparams=URI.CONNECT.SAPRFC.readparams;

class SapInputParamsConfig extends React.Component {
  constructor(props) {
    super(props);
    this.id=this.props.id;
    this.parentData=this.props.inParamsDocs||[];
    this.state = {
      curEditIndex:-1,
  ...
  saveData = () => {
        ...
        AjaxUtils.post(SubmitUrl,{id:this.id,inputParams:JSON.stringify(this.state.data)},(data)=>{
  ...
  readparams=()=>{
    ...
    AjaxUtils.post(readparams,{id:this.id},(data)=>{
        ...
        if(data.state===false){
          ...
          this.setState({data:data.inputParams});
        }
    });
  }
  ...
  render() {
    ...
    const columns=[{
      ...
      render: (text, record, index) =>this.renderEditTextArea(index,'paramsValue',text),
      width:'20%',
    }];
    ...
    return (
      <div>
        ...
        <Button  onClick={this.readparams}  type="ghost" icon='arrow-up' style={{marginBottom:'5px'}} >导入输入参数</Button> {' '}
        <Table
...
export default SapInputParamsConfig;
