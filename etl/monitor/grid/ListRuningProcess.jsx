...
const RUN_URL=URI.ETL.MONITOR.runProcessInstance;
const LIST_LOGDB=URI.ETL.MONITOR.listETLLogDbName;

class ListRuningProcess extends React.Component {
  constructor(props) {
    super(props);
    this.appId=this.props.appId;
    this.applicationId=this.props.applicationId;
    this.url=LIST_URL+"?runType=1&applicationId="+this.applicationId; //1表示运行中流程
    this.state={
  ...
  loadData=(pagination=this.state.pagination, filters={}, sorter={})=>{
    let url=this.url+"&logDbName="+this.state.logDbName;
    ...
    if(this.state.selectedRowKeys.length===0){
      ...
      AjaxUtils.post(RUN_URL,postData,(data)=>{
        ...
        if(data.state===false){
          ...
          AjaxUtils.showInfo("成功运行("+data.msg+")条流程!");
          this.loadData();
        }
      });
    }
  ...
  addTabPane=(id,name,record)=>{
      ...
      let content;
      if(id==='Process'){
        content=(<ProcessMonitor logDbName={this.state.logDbName} status='current' processId={record.processId} transactionId={record.transactionId} appId={this.appId}  close={this.closeCurrentTab} />);
      }else{
        return;
      }
    ...
    return (
      <div>
            <Tabs
              ...
              hideAdd={true}
            >
            <TabPane tab="未结束任务" key="home" style={{padding:'0px'}}>
              <Row style={{marginBottom:5}} gutter={0} >
                <Col span={12} >
                <ButtonGroup>
                  <Button  type="primary" onClick={AjaxUtils.showConfirm.bind(this,"注意:只能结束本服务器运行的线程！","偿试把执行本流程的线程停止执行,如果重启了应用服务器请不要执行本操作!可直接标记为结束即可!",this.endProcessThread)}  disabled={!hasSelected} icon="poweroff"   >偿试停止线程运行</Button>
                  <Button  type="ghost" onClick={this.endProcessStatus}  disabled={!hasSelected} icon="check"   >标记为结束</Button>
                  <Button  type="ghost" onClick={this.showConfirm} icon="delete"  disabled={!hasSelected} >删除记录</Button>
...
export default ListRuningProcess;
