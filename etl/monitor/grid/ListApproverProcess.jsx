...
import ListProcessApproveData from './ListProcessApproveData';
import ApproverForm from '../form/ApproverForm';
...
class ListApproverProcess extends React.Component {
  constructor(props) {
    super(props);
    this.appId=this.props.appId;
    this.applicationId=this.props.applicationId;
    this.url=LIST_URL+"?runType=7&applicationId="+this.applicationId; //7表示待审批流程
    this.state={
  ...
  render(){
    ...
    const columns=[
      {
        ...
        sorter:true,
          ...
          width: '15%',
          render: (text,record) => {return record.currentStatus+"("+record.approverUserIds+")"}
      },{
          title: '流程图监控',
          dataIndex: 'process',
          width: '10%',
    ...
    const expandedRow=(record)=>{
      return (
        <Card bordered={true} >
          <ListProcessApproveData transactionId={record.transactionId} processId={record.processId} nodeId={record.currentNodeId} />
        </Card>
        );
    }
    ...
    return (
      <div>
            <Modal key={Math.random()} title='数据审批' maskClosable={false}
              ...
              <ApproverForm ids={this.state.selectedRowKeys.join(",")}  close={this.closeModal} />
            </Modal>
            <Tabs
              ...
              hideAdd={true}
            >
            <TabPane tab="待手动审批任务" key="home" style={{padding:'0px'}}>
              <Row style={{marginBottom:5}} gutter={0} >
                <Col span={12} >
                <ButtonGroup>
                  <Button  type="primary" onClick={this.showModal}  disabled={!hasSelected} icon="check"   >审批</Button>
...
export default ListApproverProcess;
