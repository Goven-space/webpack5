...
import ExcelReadNodeColumns from './components/ExcelReadNodeColumns';
        ...
        <Tabs size="large">
          <TabPane  tab="基本属性" key="props"  >
              ...
              >
                ...
                (
                  (<Select  >
                  <Option value='1'>指定具体的Excel文件</Option>
              ...
              >
                {getFieldDecorator('filePath',{rules: [{ required: false}],initialValue:'d:/xls'})
                (
                  (<Input />)
                )}
              </FormItem>
              ...
              >{
                getFieldDecorator('fileExtension',{rules: [{ required:false}],initialValue:'xlsx,xls,csv'})
                (<Input />)
                }
              </FormItem>
              <FormItem
              ...
              >
                {getFieldDecorator('fileName',{rules: [{ required: false}],initialValue:'excel.xlsx'})
                (
                  (<Input />)
                )}
              </FormItem>
              ...
              <FormItem label="工作表序号"
                labelCol={{ span: 4 }}
                wrapperCol={{ span: 16 }}
                help='支持${变量},指定数据所在工作表的序号第一个Sheet1为1第二个为2依次类推每次只能读取一个工作表中的数据'
              >
                {getFieldDecorator('sheetNo',{rules: [{ required: true}],initialValue:'1'})
                (
                    (<Input   />)
                )}
              </FormItem>
          ...
          <TabPane  tab="输入字段" key="fieldConfig"  >
              <ExcelReadNodeColumns form={this.props.form} applicationId={this.applicationId} tableColumns={this.state.formData.tableColumns} processId={this.processId} ref='tableColumns' />
              <FormItem label=""
                labelCol={{ span: 4 }}
                wrapperCol={{ span: 16 }}
              >
                ...
                (
                  <RadioGroup>
                    <Radio value='2'>自定义字段(一一对应Excel中的列)</Radio>
                    <Radio value='1'>Excel的第一行作为字段名</Radio>
                    <Radio value='3'>使用Excel的行列标识作为字段名</Radio>
                  </RadioGroup>
                )
                }
              </FormItem>
