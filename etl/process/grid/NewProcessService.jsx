...
const SubmitUrl=URI.ETL.CONFIG.publish; //发布API的url
...
class form extends React.Component{
  constructor(props){
    ...
    this.configId=this.props.configId; //流程配置的id
    this.configName=this.props.configName; //配置的名称
          ...
          AjaxUtils.post(SubmitUrl,postData,(data)=>{
              ...
              if(data.state===false){
                ...
                if(data.msg.indexOf('(0)')!==-1){
                  Modal.error({title: '接口创建失败',content:data.msg+",URI重复或者其他原因!",width:600});
                }else{
                  Modal.info({title: '接口创建结果',content:data.msg,width:600});
                }
  ...
  render() {
    ...
    const getByFiltersServiceUrl="/"+this.appId.toLowerCase()+"/"+this.configId.toLowerCase();
    ...
    <Spin spinning={this.state.mask} tip="Loading..." >
      <Form onSubmit={this.onSubmit} >
        ...
        <FormItem
          label="参数类型"
          help='给流程提交数据时请选择Body请求,查询数据可选择Form Query'
          {...formItemLayout4_16}
        >
          {
        ...
        <FormItem
          label="异步"
          help='指定流程是否异步启动,默认同步模式等待所有节点执行完成后返回结果'
          {...formItemLayout4_16}
        >
          {
        ...
        <FormItem
          label="上传附件"
          help='是否允许API上传一个附件(txt,excel等)给ETL流程进行处理'
          {...formItemLayout4_16}
        >
          {
            getFieldDecorator('uploadFileFlag',{initialValue:'false'})
            ( <RadioGroup>
        ...
        <FormItem
          label="返回结果"
          help='是表示把数据处理后的结果返回给调用端'
          {...formItemLayout4_16}
        >
          {
            getFieldDecorator('returnData',{initialValue:'0'})
            ( <RadioGroup>
...
const NewProcessService = Form.create()(form);

export default NewProcessService;
