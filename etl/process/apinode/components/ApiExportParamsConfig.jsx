...
const parseExportParamsUrl=URI.ETL.API.parseExportParams;
...
class ApiExportParamsConfig extends React.Component {
  ...
  render() {
    ...
    const columns=[{
      title: '字段Id',
      dataIndex: 'paramsId',
      render: (text, record, index) => this.renderEditText(index,'paramsId', text,"流字段Id"),
      width:'25%',
    },{
      title: '字段取值',
      dataIndex: 'paramsValue',
      render: (text, record, index) =>this.renderEditText(index,'paramsValue',text,"使用JsonPath取API返回的JSON中的值如:$.data.userid"),
      width:'65%',
    },{
      title: '操作',
      dataIndex: 'action',
    ...
    return (
      <div>
        <Modal  title='从API响应JSON中自动分析' maskClosable={false}
            width='1000px'
            style={{ top: 20 }}
        ...
        <Button  onClick={this.showLoadFromJsonModal}  type="ghost"   icon='arror-up'  style={{marginBottom:'5px'}} >从结果JSON中自动分析</Button>
        <Table
