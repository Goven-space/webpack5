...
import FieldMappingConfig_Table from './components/FieldMappingConfig_Table';
...
class form extends React.Component{
  ...
  getFieldMapConfigs=()=>{
    return this.state.formData.tableColumns;
  }
      ...
      <Form onSubmit={this.onSubmit} >
        <Tabs size="large">
          <TabPane  tab="基本属性" key="props"  >
              ...
              <FormItem label="源字段匹配模式" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}>
                {getFieldDecorator('patternFlag',{initialValue:'0'})
                (
                  <RadioGroup>
                    <Radio value='0'>原字段Id精确匹配</Radio>
                    <Radio value='1'>使用正规表达式匹配原字段Id(如:A(.*)表示以A开头)</Radio>
                  </RadioGroup>
                )}
              </FormItem>
              <FormItem
          ...
          <TabPane  tab="字段映射配置" key="field_table" >
            <div style={{height:'550px',width:'1000px',overflow:'auto'}} >
              <FieldMappingConfig_Table
                ...
                />
                注意:字段映射后如果需要删除原字段请选中删除，如果同时要映射多个目标字段请直接用逗号分隔
              </div>
          </TabPane>
          <TabPane  tab="字段映射关系图" key="field_flow" >
            <div style={{height:'550px',width:'1000px',overflow:'auto'}} >
              <FieldMappingConfig
                parentForm={this.props.form}
                processId={this.processId}
                pNodeId={this.nodeObj.key}
                updateFieldMapConfigs={this.updateFieldMapConfigs}
                getFieldMapConfigs={this.getFieldMapConfigs}
                />
              </div>
          </TabPane>
        </Tabs>
