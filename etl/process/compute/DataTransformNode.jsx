...
import DataTransformFieldConfig from './components/DataTransformFieldConfig';
...
class form extends React.Component{
  constructor(props){
    ...
    this.state={
      mask:false,
      formData:{tableColumns:'[]',filtersTableColumns:'[]'},
    };
  }
      ...
      <Form onSubmit={this.onSubmit} >
        <Tabs size="large">
          <TabPane  tab="基本属性" key="props"  >
              ...
              <FormItem
                label="最大转换记录数"
                help='只对符合条件的top n条记录进行转换,0表示不限定'
                labelCol={{ span: 4 }}
                wrapperCol={{ span: 16 }}
              >{
                getFieldDecorator('maxCount',{rules: [{ required:false}],initialValue:0})
                (<InputNumber min={0}  />)
                }
              </FormItem>
              <FormItem
          ...
          <TabPane  tab="过滤条件" key="condition" >
            ...
            <FormItem labelCol={{ span: 2 }} wrapperCol={{ span: 20 }}
              help='只有符合条件的记录才会进行清洗则则的运算(排除不需要运算的数据可以加快清洗规则的执行速度)不配置过滤条件则会对所有数据行执行清洗运算'
              >
          ...
          <TabPane  tab="清洗规则" key="field" >
            <div>
              <DataTransformFieldConfig
                ...
                />
                说明:支持绑定(数据清洗转换规则、数据脱敏规则、数据生成规则)
            </div>
          </TabPane>
        </Tabs>
