...
class DataGenerationNodeConfig extends React.Component {
  constructor(props) {
    super(props);
  ...
  deleteRow=(id,key)=>{
    ...
    if(id!==undefined && id!==""  && id!==null ){
      let data=this.state.data.filter((dataItem) => dataItem.colId!==id);
      this.setState({data});
    }
  }
  ...
  render() {
    ...
    let columns=[{
      ...
      width:'25%',
      render: (text, record, index) => this.renderEditText(index,'defaultValue', text,record,"用||分隔表示随机取一个值"),
    },{
      title: '绑定数据生成规则',
      dataIndex: 'ruleId',
      width:'30%',
      ...
      }},
    {
      ...
      width:'5%',
      render: (text, record, index) => {
        return (<span><a onClick={() => this.deleteRow(record.colId,record.key)}>删除</a></span>);
      }
    }
    ];
...
export default DataGenerationNodeConfig;
