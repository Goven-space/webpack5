    ...
    <Spin spinning={this.state.mask} tip="Loading..." >
        ...
        <Tabs size="large">
          <TabPane  tab="基本属性" key="props"  >
              ...
              <FormItem label="动态数据源" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
                 help='数据源ID不固定由变量确定'
              >
                {getFieldDecorator('dynamicSourceFlag',{initialValue:"0"})
                (
                  <RadioGroup>
              ...
              <FormItem
                label="数据源变量Id"
                help='指定数据源来自于那一个变量(支持局部或全局变量)'
                labelCol={{ span: 4 }}
                wrapperCol={{ span: 16 }}
                style={{display:this.props.form.getFieldValue("dynamicSourceFlag")==='1'?'':'none'}}
              >{
                getFieldDecorator('variantDbSourceId',{initialValue:'P_MODEL_DATASRCID'})
                (<Input  />)
                }
              </FormItem>
              <FormItem
              ...
              <FormItem
                label="自定义Insert"
                help='自定义insert into语句如:insert delayed into tableName,insert /*+append*/ into tableName等，默认为insert into'
                labelCol={{ span: 4 }}
                wrapperCol={{ span: 16 }}
              >{
              ...
              <FormItem label="清空数据流" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
                 help='数据提交后立即清空数据流,可以提升内存使用率,清空后数据流不再流入下一节点'
              >
          ...
          <TabPane  tab="输出选项" key="outConfig"  >
            ...
            <FormItem label="日记策略" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }} help='数据流保存策略(注意:如果基本属性中选择了清空数据流则本策略将失效)' >
