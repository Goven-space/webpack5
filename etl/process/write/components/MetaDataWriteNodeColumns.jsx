...
class MetaDataWriteNodeColumns extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const columns=[{
      ...
      width:'16%',
      render: (text, record, index) => this.renderDefaultValue(index, 'defaultValue', text),
    },{
      title: '事件',
      dataIndex: 'eventCode',
      width:'10%',
    ...
    return (
      <div>
          <div style={{paddingBottom:10}} >
              <ButtonGroup >
                <Button type='primary' onClick={AjaxUtils.showConfirm.bind(this,"导入元数据中的字段?","如果当前已经有字段配置数据,导入字段会丢失现有数据!",this.loadTableColumns.bind(this))}  icon="select"  >从元数据中载入</Button>
...
export default MetaDataWriteNodeColumns;
