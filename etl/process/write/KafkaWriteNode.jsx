...
const connectionsUrl=URI.CORE_DATASOURCE.listAll+"?configType=Kafka";
        ...
        <Tabs size="large">
          <TabPane  tab="基本属性" key="props"  >
            ...
            <FormItem
              ...
              labelCol={{ span: 4 }}
              wrapperCol={{ span: 16 }}
              help="请选择一个Kafka数据源(请在数据源管理中进行链接配置)"
            >{
              getFieldDecorator('dbConnId',{rules: [{ required: true}],initialValue:'kafka'})
            ...
            <FormItem
              ...
              help="指定要发送消息的topic"
            >{
              getFieldDecorator('topic',{rules: [{ required: true}],initialValue:'etl'})
              (<Input />)
              }
            </FormItem>
						<FormItem
                  ...
                  labelCol={{ span: 4 }}
                  wrapperCol={{ span: 16 }}
                  help='指定每次写入的数据量,0表示一次全部写入'
                >{
                    getFieldDecorator('quantity', { rules: [{ required: false }],initialValue: 0 })
                  ...
                  help='默认分区为0'
                >{
                    getFieldDecorator('partition', { rules: [{ required: false }], initialValue: 0 })
                      (<InputNumber style={{width:'100%'}}/>)
                  }
            </FormItem> */}
            <FormItem
              ...
              labelCol={{ span: 4 }}
              wrapperCol={{ span: 16 }}
              help='使用JSONPath取值如：$.data可以当前数据流中的data字段，空表示默认输出当前数据流'
            >{
              getFieldDecorator('jsonPath',{rules: [{ required: false}]})
              (<Input />)
              }
            </FormItem>
