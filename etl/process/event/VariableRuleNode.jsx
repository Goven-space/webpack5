        ...
        <Tabs size="large">
          <TabPane  tab="基本属性" key="props"  >
              ...
              <FormItem label="变量类型" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
                 help='设为全局变量则所有节点均可获取,局部变量一般只传给后继节点使用'
              >
                {getFieldDecorator('gobalParams',{initialValue:'1'})
                (
                  <RadioGroup>
                    <Radio value='1'>设为全局变量</Radio>
                    <Radio value='0'>局部变量</Radio>
                  </RadioGroup>
                )}
              </FormItem>
              <FormItem label="设置到数据流" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
                 help='把变量设置到data流中的每一行数据中'
              >
                {getFieldDecorator('setToRowsData',{initialValue:'0'})
                (
                  <RadioGroup>
          ...
          <TabPane  tab="变量设置" key="field" >
            <div>
              <VariableRuleNodeParams
                ...
                />
              {"变量值支持取局部变量、全局变量、data数据:${变量id} ${$.data[0].id}、公共变量:${$config.变量id}"}
            </div>
          </TabPane>
        </Tabs>
