  ...
  onSubmit = (closeFlag) => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
          ...
          let writeType=postData.writeType;
          if(writeType==='1'){writeType="逐条判断输出";}
          else if(writeType==='2'){writeType="批量插入";}
          else if(writeType==='3'){writeType="合并后批量";}
    ...
    <Spin spinning={this.state.mask} tip="Loading..." >
        ...
        <Tabs size="large">
          ...
          <TabPane  tab="输出选项" key="outConfig"  >
            <FormItem label="创建目标表" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
               help='目标表不存在时自动创建'
            >
              {getFieldDecorator('createTable',{initialValue:'1'})
              (
                <RadioGroup>
                  <Radio value='1'>是</Radio>
                  <Radio value='0'>否</Radio>
            ...
            <FormItem label="更新选项" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
              style={{display:this.props.form.getFieldValue("writeType")==='2'?'none':''}}
          ...
          <TabPane  tab="事务设置" key="nodeTransactionFlag"  >
            ...
            <FormItem label="关闭链接" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
               help='本节点传输单个表或分页数据后是否立即关闭数据库链接,默认为关闭并提交所有数据'
            >
          ...
          <TabPane  tab="结果断言" key="resultAssert"  >
            ...
            >
              ...
              (
                <RadioGroup>
                  <Radio value='1'>终止传输</Radio>
                  <Radio value='0'>继续运行</Radio>
                </RadioGroup>
              )}
            </FormItem>
          </TabPane>
