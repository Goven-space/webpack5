...
import SgingleTableTargetField from './components/SgingleTableTargetField';
        ...
        <Tabs size="large" style={{display:current===0?'':'none'}} >
          ...
          <Tabs size="large" style={{display:current===2?'':'none'}} >
            <TabPane  tab="目标数据库表" key="targettable"  >
                ...
                >
                  {
                    getFieldDecorator('targetDataSourceId',{  rules: [{ required: true}],initialValue:''})
                    (<TreeNodeSelect url={dataSourceSelect} options={{showSearch:true,multiple:false,allowClear:true,treeNodeFilterProp:'label',searchPlaceholder:'输入搜索关键字'}}  />)
                  }
                </FormItem>
                <FormItem
                ...
                <FormItem
                  ...
                  wrapperCol={{ span: 16 }}
                >{
                  getFieldDecorator('commitCount',{rules: [{ required:false}],initialValue:10000})
                  (<InputNumber min={0}  />)
                  }
                </FormItem>
                <FormItem
            ...
            <TabPane  tab="预览数据" key="targetDataPreview"  >
