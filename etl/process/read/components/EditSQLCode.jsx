...
class EditSQLCode extends React.Component{
  ...
  render() {
    let helpInfo='提示:使用${lastStartTime},${lastEndTime},${lastSuccessStartTime},${lastSuccessEndTime}获取流程最后一次运行开始和结束时间(格式为:yyyy-MM-dd HH:mm:ss),使用${局部|全局|http变量Id},${$config.公共变量Id}可以引用变量,系统默认使用游标分页对于大数据量读取请使用SQL分页，SQL分页支持:${beginRow},${endRow},${pageSize}三个变量可在SQL中使用';
    return (
