...
class MetaDataReadNodeColumns extends React.Component {
  ...
  loadTableColumns=()=>{
    ...
    if(metaDataConfigId===''){message.error("请先指定元数据模型再执行本操作!");return;}
    let formData=this.form.getFieldsValue();
    this.setState({loading:true});
    ...
    return (
      <div>
          <div style={{paddingBottom:10}} >
              <ButtonGroup >
              <Button type='primary' onClick={AjaxUtils.showConfirm.bind(this,"导入元数据字段?","如果当前已经有字段配置数据,导入字段会丢失现有数据!",this.loadTableColumns.bind(this))}  icon="select"  >同步元数据中的字段</Button>
