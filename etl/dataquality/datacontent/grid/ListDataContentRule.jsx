...
import {Table,Card,Icon,Menu,Dropdown,Popconfirm,Button,Modal,Input,Row,Col,Tag,Layout,Divider,Tabs} from 'antd';
import * as URI from '../../../../core/constants/RESTURI';
import * as AjaxUtils from '../../../../core/utils/AjaxUtils';
import * as GridActions from '../../../../core/utils/GridUtils';
import NewDataContentRule from '../form/NewDataContentRule';
import ListDataContentLog from './ListDataContentLog';
import ListDataContentErrorData from './ListDataContentErrorData';
...
const LIST_URL=URI.ETL.DATAQUALITY_DATACONTENT.list;
const DELETE_URL=URI.ETL.DATAQUALITY_DATACONTENT.delete;
const RUN_URL=URI.ETL.DATAQUALITY_DATACONTENT.run;
const startTaskJobUrl=URI.ETL.DATAQUALITY_DATACONTENT.start;
const stopTaskJobUrl=URI.ETL.DATAQUALITY_DATACONTENT.stop;
const TabPane = Tabs.TabPane;

class ListDataContentRule extends React.Component {
  constructor(props) {
    super(props);
  ...
  render(){
    ...
    const columns=[{
        ...
        width: '20%',
          ...
          sorter: true,
          width: '20%',
          render: (text,record) => {return text+'.'+record.tableName;}
      },{
        title: '执行时间',
      ...
      const expandedRow=(record)=>{
        return (
          <div style={{width:this.state.width,backgroundColor:'#ffffff',border:'1px solid #f4f4f4',padding:8}}>
            <Tabs size='large'>
              <TabPane tab="检测历史" key="log" style={{padding:'0px'}}>
                <ListDataContentLog id={record.id} />
              </TabPane>
              <TabPane tab="错误数据" key="errordata" style={{padding:'0px'}}>
                <ListDataContentErrorData id={record.id} />
              </TabPane>
            </Tabs>
          </div>
          );
    ...
    return (
      <div >
    	          <Modal key={Math.random()} maskClosable={false}
                    ...
                    <NewDataContentRule id={currentId} appId={this.props.appId} applicationId={this.applicationId} categoryId={this.categoryId} close={this.closeModal} />
                </Modal>
...
export default ListDataContentRule;
