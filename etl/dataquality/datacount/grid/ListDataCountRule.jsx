...
import NewDataCountRule from '../form/NewDataCountRule';
import ListDataCountLog from './ListDataCountLog';
...
const LIST_URL=URI.ETL.DATAQUALITY_DATACOUNT.list;
const DELETE_URL=URI.ETL.DATAQUALITY_DATACOUNT.delete;
const RUN_URL=URI.ETL.DATAQUALITY_DATACOUNT.run;
const startTaskJobUrl=URI.ETL.DATAQUALITY_DATACOUNT.start;
const stopTaskJobUrl=URI.ETL.DATAQUALITY_DATACOUNT.stop;

class ListDataCountRule extends React.Component {
  constructor(props) {
    super(props);
  ...
  run=(id)=>{
      ...
      AjaxUtils.post(url,{id:id},(data)=>{
          ...
          if(data.state===false){
            ...
            AjaxUtils.showInfo("执行成功，请点击+号查看对比记录!");
            this.loadData();
          }
      });
  }
  ...
  render(){
    ...
    const columns=[{
        ...
        width: '20%',
          ...
          sorter: true,
          width: '15%',
          render: (text,record) => {return text+'.'+record.sourceTableName;}
      },{
            title: '目标表',
            dataIndex: 'targetDataSourceId',
            sorter: true,
            width: '15%',
            render: (text,record) => {return text+'.'+record.targetTableName;}
      },{
        title: '执行时间',
    ...
    return (
      <div >
    	          <Modal key={Math.random()} maskClosable={false}
                    ...
                    <NewDataCountRule id={currentId} appId={this.props.appId} applicationId={this.applicationId} categoryId={this.categoryId} close={this.closeModal} />
                </Modal>
...
export default ListDataCountRule;
