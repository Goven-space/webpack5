...
const loadDataUrl=URI.ETL.DATAQUALITY_DATACHANGE.getById;
const saveDataUrl=URI.ETL.DATAQUALITY_DATACHANGE.save;
...
class form extends React.Component{
  ...
  componentDidMount(){
    ...
    if(id===undefined || id===''){
        ...
        this.state.formData.minCount=0;
        this.state.formData.maxCount=1000;
        ...
        <Tabs size="large">
          <TabPane  tab="规则配置" key="props"  >
            ...
            <FormItem
              ...
              labelCol={{ span: 4 }}
              wrapperCol={{ span: 16 }}
              help='对比上一次检测结果，当数据量变化少于指定值时发送通知'
            >{
              getFieldDecorator('minCount',{rules: [{ required: true}],initialValue:"0"})
              (<InputNumber min={0} />)
              }
            </FormItem>
            <FormItem
              ...
              labelCol={{ span: 4 }}
              wrapperCol={{ span: 16 }}
              help='对比上一次检测结果，当数据量变化大于指定值时发送通知'
            >{
              getFieldDecorator('maxCount',{rules: [{ required: true}],initialValue:"0"})
              (<InputNumber min={0} />)
              }
            </FormItem>
            <FormItem  label="SQL"  labelCol={{ span: 4 }} wrapperCol={{ span: 18 }} help='自定义统计SQL如:select count(*) from table where id>1' >
              {
          ...
          >
            ...
            (
              <Select>
                ...
                <Option value='2'>仅数据量超出范围时发送通知</Option>
              </Select>
            )}
          </FormItem>
          <FormItem
