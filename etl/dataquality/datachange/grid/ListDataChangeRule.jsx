...
import NewDataChangeRule from '../form/NewDataChangeRule';
import ListDataChangeLog from './ListDataChangeLog';
...
const LIST_URL=URI.ETL.DATAQUALITY_DATACHANGE.list;
const DELETE_URL=URI.ETL.DATAQUALITY_DATACHANGE.delete;
const RUN_URL=URI.ETL.DATAQUALITY_DATACHANGE.run;
const startTaskJobUrl=URI.ETL.DATAQUALITY_DATACHANGE.start;
const stopTaskJobUrl=URI.ETL.DATAQUALITY_DATACHANGE.stop;

class ListDataChangeRule extends React.Component {
  constructor(props) {
    super(props);
  ...
  render(){
    ...
    const columns=[{
        ...
        dataIndex: 'minCount',
        width: '15%',
        render: (text,record) => {return text+'<'+record.maxCount;}
      },{
        title: '执行时间',
      ...
      const expandedRow=(record)=>{
        return (
          <div style={{width:this.state.width,backgroundColor:'#ffffff',border:'1px solid #f4f4f4',padding:8}}>
                <ListDataChangeLog id={record.id} />
          </div>
          );
      }
    ...
    return (
      <div >
    	          <Modal key={Math.random()} maskClosable={false}
                    ...
                    <NewDataChangeRule id={currentId} appId={this.props.appId} applicationId={this.applicationId} categoryId={this.categoryId} close={this.closeModal} />
                </Modal>
...
export default ListDataChangeRule;
