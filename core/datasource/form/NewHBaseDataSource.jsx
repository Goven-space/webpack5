...
class form extends React.Component{
  ...
  insertDemo=()=>{
        let code=`hbase.zookeeper.quorum=127.0.0.1:2181
hbase.client.keyvalue.maxsize=60485760`;
    ...
    <Spin spinning={this.state.mask} tip="Loading..." >
      <Form onSubmit={this.onSubmit} >
        ...
        <FormItem
          ...
          labelCol={{ span: 4 }}
          wrapperCol={{ span: 16 }}
          help="指定一个唯一Id在获取数据库链接时使用(默认HBase的id为HBase)"
        >
          {
            getFieldDecorator('configId', {
              rules: [{ required: true}],initialValue:'HBase'
            })
            (<Input />)
          }
        </FormItem>
        <FormItem
          ...
          labelCol={{ span: 4 }}
          wrapperCol={{ span: 16 }}
          help={<span><a style={{cursor:'pointer'}} onClick={this.insertDemo}>生成示例</a></span>}
        >{
          getFieldDecorator('props',{initialValue:``})
          (<Input.TextArea autosize={{ minRows: 12, maxRows: 30 }} />)
          }
        </FormItem>
        <FormItem
          label="备注"
...
const NewHBaseDataSource = Form.create()(form);

export default NewHBaseDataSource;
