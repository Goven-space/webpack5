    ...
    <Spin spinning={this.state.mask} tip="Loading..." >
      <Form onSubmit={this.onSubmit} >
        ...
        <FormItem
          ...
          labelCol={{ span: 4 }}
          wrapperCol={{ span: 16 }}
          help="指定一个唯一Id在获取链接时使用(如:ActiveMq)"
        >
          {
            getFieldDecorator('configId', {
              rules: [{ required: true}],initialValue:'jms'
            })
            (<Input />)
          }
        </FormItem>
        <FormItem label="JMS类型" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='请指定JMS的具体类型'
        >
          {getFieldDecorator('jmsType',{initialValue:'ActiveMQ'})
          (
            (<Select >
        ...
        <FormItem label="服务器主机" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='ActiveMQ格式为(tcp://ip:port),IBM MQ格式为(ip:port:channel:queueManager:ccsid)'
        >
          {getFieldDecorator('jdbcUrl',{rules: [{ required: true}],initialValue:'tcp://192.107.60.1:61616'})
          (
            (<Input />)
          )}
        </FormItem>
        <FormItem label="用户Id" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定链接Jms服务器链接的用户Id'
        >
          {getFieldDecorator('userId',{rules: [{ required: true}],initialValue:'admin'})
          (
            (<Input />)
          )}
        </FormItem>
        ...
        <FormItem
          ...
          help='指定其他链接属性'
        >{
          getFieldDecorator('props',{initialValue:`pubSubDomain=true\ndeliveryMode=2\nreceiveTimeout=1000`})
