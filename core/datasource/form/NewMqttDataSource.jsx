    ...
    <Spin spinning={this.state.mask} tip="Loading..." >
      <Form onSubmit={this.onSubmit} >
        ...
        <FormItem
          ...
          labelCol={{ span: 4 }}
          wrapperCol={{ span: 16 }}
          help="指定一个唯一Id在获取数据库链接时使用(默认MQTT的id为mqtt)"
        >
          {
            getFieldDecorator('configId', {
              rules: [{ required: true}],initialValue:'mqtt'
            })
            (<Input />)
          }
        </FormItem>
        <FormItem label="服务器主机" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定链接MQTT服务的协议和主机'
        >
          {getFieldDecorator('jdbcUrl',{rules: [{ required: true}],initialValue:'tcp://127.0.0.1:6110'})
          (
            (<Input />)
          )}
        </FormItem>
        <FormItem label="用户Id" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定链接MQTT服务器的用户Id'
        >
        ...
        <FormItem label="客户机标识" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定本客户机的ClientId标识'
        >
          {getFieldDecorator('dataSource',{rules: [{ required: true}],initialValue:'client01'})
          (
            (<Input />)
          )}
        </FormItem>
        <FormItem
          ...
          help='指定其他链接属性'
        >{
          getFieldDecorator('props',{initialValue:`ConnectionTimeout=10\nKeepAliveInterval=20`})
