  ...
  onSubmit = (closeFlag,testConn='') => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
          ...
          postData.configType='RocketMQ_Local';
          this.setState({mask:true});
    ...
    <Spin spinning={this.state.mask} tip="Loading..." >
      <Form onSubmit={this.onSubmit} >
        ...
        <FormItem
          ...
          labelCol={{ span: 4 }}
          wrapperCol={{ span: 16 }}
          help="指定一个唯一Id在获取数据库链接时使用"
        >
          {
            getFieldDecorator('configId', {
              rules: [{ required: true}],initialValue:'rocketmq'
            })
            (<Input />)
          }
        </FormItem>
        <FormItem label="RocketMQ主机" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定链接RocketMQ服务器主机'
        >
          {getFieldDecorator('virtualHost',{rules: [{ required: true}],initialValue:'127.0.0.1'})
          (
            (<Input />)
          )}
        </FormItem>
        <FormItem label="Group ID" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='一个 Group ID 代表一个 Consumer 实例群组'
        >
          {getFieldDecorator('schemaUserId',{rules: [{ required: true}],initialValue:'GID_'})
          (
            (<Input />)
          )}
        </FormItem>
        <FormItem label="AccessKeyId" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定阿里云的AccessKeyId'
        >
          {getFieldDecorator('accessKeyId',{initialValue:''})
          (
            (<Input type='password' />)
          )}
        </FormItem>
        <FormItem label="AccessKeySecret" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定阿里云的AccessKeySecret'
        >
          {getFieldDecorator('accessKeySecret',{initialValue:''})
          (
            (<Input type='password' />)
          )}
        </FormItem>
        <FormItem label="超时时间" labelCol={{ span: 4 }} wrapperCol={{ span: 16 }}
          help='指定超时时间'
        >
          {getFieldDecorator('maxConnectNum',{rules: [{ required: true}],initialValue:'3000'})
          (
            (<Input />)
          )}
        </FormItem>
