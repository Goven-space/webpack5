...
import ListSqlConfigByModelId from './ListSqlConfigByModelId';
...
class EditEntryColumnsConfig extends React.Component {
  ...
  render() {
    ...
    const columns=[{
      ...
      title: '隐藏',
      dataIndex: 'editHidden',
      width:'5%',
      render: (text, record, index) => this.renderCheckBox(index,'editHidden', text),
    },{
      title: '事件',
        ...
        <Tabs defaultActiveKey="columnsConfig" size='large'  >
            <TabPane tab="字段配置" key="columnsConfig" animated={false}>
              ...
              <Radio.Group  value={this.state.hiddenType} onChange={this.handleRadioChange} >
                <Radio.Button  value="AllShow">显示所有列 </Radio.Button>
                <Radio.Button  value="AllHidden">隐藏所有列</Radio.Button>
              </Radio.Group>
              </div>
              <Table
              bordered
            ...
            <TabPane tab="创建的API" key="ServiceList" animated={false}>
              <ListServicesByModelId appId={this.appId}  filters={{modelId:[this.modelId]}} />
            </TabPane>
            <TabPane tab="关联的SQL" key="SqlConfigList" animated={false}   >
                <ListSqlConfigByModelId appId={this.appId} modelId={this.modelId}  />
            </TabPane>
        </Tabs>
      </div>
      );
