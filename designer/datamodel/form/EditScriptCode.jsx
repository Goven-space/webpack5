...
class EditScriptCode extends React.Component{
  constructor(props){
    super(props);
  ...
  //覆盖本地工程文件中的源码代码
    ...
    AjaxUtils.post(url,{beanId:this.record.configId,appId:this.record.appId,code:code,codeType:'sql'},(data)=>{
      this.setState({mask:false});
      if(data.state===false){
  ...
  readFileCode=()=>{
    this.setState({mask:true,visible: false});
    let url=readCodeUrl+"?beanId="+this.record.configId+"&appId="+this.record.appId+"&codeType=sql";
    AjaxUtils.get(url,(data)=>{
      this.setState({mask:false});
      if(data.state===false){
  ...
  readFromRepository=(remote)=>{
    ...
    AjaxUtils.showConfirm("更新代码确认",msg,()=>{
      let url=checkoutUrl+"?configId="+this.record.configId+"&appId="+this.record.appId+"&codeType=sql&remote="+remote;
  ...
  commitToRepository=(e)=>{
    ...
    AjaxUtils.post(commitUrl,{configId:this.record.configId,remote:remote,appId:this.record.appId,code:this.getCode,msg:this.state.commitInfo,codeType:'sql'},(data)=>{
      this.setState({mask:false});
      if(data.state===false){
      ...
      <Spin spinning={this.state.mask} tip="Loading..." >
          ...
          <div style={{margin:'0 0 5px 0',padding:'5px',border:'solid 1px #ccc',borderRadius:'5px'}}>
            <ButtonGroup>
              ...
              {
                this.codeType==='java'?<Button icon='check-circle-o' onClick={this.saveCompile}>编译</Button>:''
              }
