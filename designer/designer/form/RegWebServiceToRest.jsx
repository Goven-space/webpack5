  ...
  onSubmit = (closeFlag) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
          ...
          postData.configType='WEBSERVICETOREST'; //标记类型
          postData.beanId="WebServiceForwardRest";
          postData.beanMethodName="callWebService"; //执行的方法名
          postData.appId=this.appId;
          postData.registerFlag=1;
          postData.webServiceRequestXmlFlag=false;
          postData.webServiceResponseXmlFlag=false;
      ...
      <Form onSubmit={this.onSubmit} >
          ...
          <div  style={{display:current===1?'':'none'}}   >
                ...
                <FormItem
                  ...
                  labelCol={{ span: 4 }}
                  wrapperCol={{ span: 16 }}
                  help='如果已绑定后端服务则只需填写路径即可如:/api/webservice未绑定时也可直接输入全路径http://ip/api/webservice,有多个时用逗号分隔或换行,${$config.server}可获取配置变量'
                >
                  {
                    getFieldDecorator('backendUrl', {
                      rules: [{ required: true}],
                      initialValue:'/api/webservice',
                    })
                    (<Input.TextArea autoSize  />)
                  }
                </FormItem>
                ...
                <FormItem
                  ...
                  wrapperCol={{ span: 16 }}
                >
                  {getFieldDecorator('webServiceRequestBody', {rules: [{ required: false, message: '' }]})
                    (<AceEditor mode='xml' width='100%' height='400px'/>)
                  }
                </FormItem>
                ...
                >
                  ...
                  (
                    <Select>
                      <Option value={1} >{'使用${变量}JsonPath取值并填充XML模板'}</Option>
              ...
              <Card>
                <Tabs size="large">
                  <TabPane  tab="输入参数" key="acl"  >
                    <Card title="示例Body(JSON)参数配置"  style={{display:this.state.requestBodyFlag?'':'none'}}>
                      <FormItem
                        label=""
                        labelCol={{ span: 2 }}
                        wrapperCol={{ span: 22 }}
