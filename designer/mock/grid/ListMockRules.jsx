...
import NewMockRule from '../form/NewMockRule';
...
const LIST_URL=URI.CORE_MOCK_RULE.list;
const DELETE_URL=URI.CORE_MOCK_RULE.delete;
const SubmitUrl=URI.CORE_MOCK_RULE.save; //保存地址
const exporConfigUrl=URI.CORE_MOCK_RULE.export;

class ListDataValidateRule extends React.Component {
  constructor(props) {
    super(props);
  ...
  onActionClick=(action,record,url)=>{
    if(action==="New"){
      this.addTabPane('New','新增数据模拟规则');
    }else if(action==="Delete"){
      this.deleteData(record.id);
    }else if(action==="Edit"){
      this.addTabPane('Edit','修改:'+record.configName,record);
    }else if(action==="EditCode"){
      this.addTabPane('EditCode','修改:'+record.configName,record);
    }
  }
  ...
  addTabPane=(id,name,record)=>{
      ...
      let content;
      if(id==='New'){
        content=(<NewMockRule id="" categoryId={this.categoryId} appId={this.props.appId}  close={this.closeCurrentTab} />);
      }else if(id==='Edit'){
        content=(<NewMockRule id={record.id} appId={this.props.appId} close={this.closeCurrentTab} />);
      }else if(id==='EditCode'){
        content=<EditJavaCode  code={record.code} record={record} saveEditCode={this.saveEditCode} templateType="MockRule"  />;
      }
  ...
  //通过ajax远程载入数据
  search=(value)=>{
    this.searchFilters= value ? {"configId":value,"confgiName":value} : {};
    this.loadData(this.defaultPagination)
  }
  ...
  render(){
    ...
    return (
            <Tabs
              ...
              hideAdd={true}
            >
            <TabPane tab="数据模拟规则" key="home" style={{padding:'0px'}}>
              <Row style={{marginBottom:5}} gutter={0} >
                <Col span={12} >
                <ButtonGroup>
                  <Button  type="primary" onClick={this.onActionClick.bind(this,'New')} icon="plus-circle-o"  >新增模拟规则</Button>
...
export default ListDataValidateRule;
