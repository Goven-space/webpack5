...
import RegWebsocket from '../../../designer/designer/form/RegWebsocket';
...
import ListApiCategoryNode from '../../grid/ListApiCategoryNode';
import ListAppPropertiesByAppId from '../../../designer/designer/grid/ListAppPropertiesByAppId';
import RegisterAPI from './form/RegisterAPI';
import ListDataMappingCategory from '../../../designer/mapping/grid/ListDataMappingCategory';
...
class ListApiByManager extends React.Component {
  ...
  componentWillReceiveProps=(nextProps)=>{
    ...
    if(nextProps.categoryId!==this.categoryId || nextProps.appId!==this.appId){
  ...
  loadData=(pagination=this.state.pagination, filters={}, sorter={})=>{
    ...
    if(this.appId=='*'||this.appId==''){
      filters.configType=['REG','WEBSERVICE','DUBBO','RestToWebService','JOIN','WEBSOCKET'];  //所有注册API列表
    }
    if(this.state.categoryId!=='*'){
      filters.categoryId=[this.state.categoryId];
    }
    if(this.state.searchKeywords!==''){
      this.search(this.state.searchKeywords,pagination);
    }else{
  ...
  addTabPane=(id,name,record)=>{
      ...
      if(id==='NewService'){
        //新增服务
        content=(<NewService appId={this.appId} categoryId={this.categoryId} closeTab={this.closeCurrentTab}/>);
      }else if(id==='RegService'){
        //注册服务
        content=(<RegService appId={this.appId} categoryId={this.categoryId} closeTab={this.closeCurrentTab}/>);
      }else if(id==='RegWebService'){
        //注册WebService
        content=(<RegWebService appId={this.appId} categoryId={this.categoryId} closeTab={this.closeCurrentTab}/>);
      }else if(id==='RegDubbo'){
        //注册dubbo
        content=(<RegDubbo appId={this.appId} categoryId={this.categoryId} closeTab={this.closeCurrentTab}/>);
      }else if(id==='JoinService'){
        //聚合服务
        content=(<NewJoinService appId={this.appId} categoryId={this.categoryId} closeTab={this.closeCurrentTab}/>);
      }else if(id==='RegRestToWebService'){
        //restfule转webservices
        content=(<RegRestToWebService appId={this.appId} categoryId={this.state.categoryId} closeTab={this.closeCurrentTab}/>);
      ...
      }else if(id==='RegWebSocket'){
        //WebSocket转发
        content=(<RegWebsocket appId={this.appId} categoryId={this.state.categoryId} closeTab={this.closeCurrentTab}/>);
      }else if(id==='EditRest'){
        //修改服务
        tabActiveKey=record.id; //这样避免重复，可以打开多个编辑Tab
        if(record.configType==='REG'){
          content=(<RegService appId={record.appId} id={record.id} closeTab={this.closeCurrentTab}/>);
        }else if(record.configType==='WEBSERVICE'){
          content=(<RegWebService appId={record.appId} id={record.id} closeTab={this.closeCurrentTab}/>);
        }else if(record.configType==='DUBBO'){
          content=(<RegDubbo appId={record.appId} id={record.id} closeTab={this.closeCurrentTab}/>);
        }else if(record.configType==='RestToWebService'){
          content=(<RegRestToWebService appId={record.appId} id={record.id} closeTab={this.closeCurrentTab}/>);
        }else if(record.configType==='WEBSERVICETOREST'){
          ...
          content=(<NewJoinService appId={record.appId} id={record.id} closeTab={this.closeCurrentTab}/>);
        }else if(record.configType=='WEBSOCKET'){
          //WebSocket转发
          content=(<RegWebsocket appId={record.appId} id={record.id} closeTab={this.closeCurrentTab}/>);
        }else{
          content=(<NewService appId={record.appId} id={record.id} closeTab={this.closeCurrentTab}/>);
        }
      }else if(id==='Test'){
        //服务测试
        ...
        content=(<Card><ShowApiDocForDev id={record.id}  appId={record.appId} closeTab={this.closeCurrentTab}/></Card>);
      }else if(id==='ListDataMappingCategory'){
        tabActiveKey="mapping_"+record.id;
        content=(<ListDataMappingCategory apiId={record.id}  appId={record.appId} closeTab={this.closeCurrentTab} />);
      }else{
  ...
  newMenuClick=(e)=>{
    ...
    }else if(e.key==='WebSocket'){
      this.addTabPane('RegWebSocket','注册WebSocket');
    }
  }
  ...
  render(){
    ...
    const expandedRow=(record)=>{
      return (
        ...
        <EditParamsConfig record={record} appId={this.appId} id={record.id} annotation='0' beanId={record.beanId} modelId={record.modelId} closeTab={this.closeCurrentTab}/>
        </Card>
        );
    }
    const columns=[{
    ...
    if(this.state.action==='Publish'){
      modalTitle="发布API";
      modalForm=<PublishAPIToProtal currentRecord={this.state.currentRecord} appId={this.appId} closeModal={this.closeModal} />;
    }else if(this.state.action==='Change'){
      modalTitle="API变更通知";
      modalForm=<APIVersionChageNotice currentRecord={this.state.currentRecord} appId={this.appId} closeModal={this.closeModal} />;
      ...
      modalTitle="绑定权限";
      modalForm=<ListSelectServiceMapPermission serviceId={this.state.serviceId} appId={this.appId} closeModal={this.closeModal} />;
    }
                ...
                <Row style={{marginBottom:5}} gutter={0} >
                  <Col span={14}>
                    <ButtonGroup>
                      ...
                      {
                        this.appId==''?'':
                        <Button  type="ghost" onClick={this.onActionClick.bind(this,"AppProperties")} icon="appstore"  >变量</Button>
                      }
                  ...
                  <Col span={10}>
                   <span style={{float:'right'}} >
                     {this.props.apiReg ?
                     <span>
                     分类:<TreeNodeSelect value={this.state.categoryId} defaultData={{label:'所有分类列表',value:'*'}} url={this.menuUrl}  onChange={this.onCategoryChange} isParentDisable={true} options={{style:{width:'200px'}}} />
                     </span>
                      : null
                     }
                搜索:<Search
