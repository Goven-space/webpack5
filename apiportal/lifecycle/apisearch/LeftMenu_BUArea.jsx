...
const MenuUrl=URI.CORE_BusinessDomain.treeOptData;

class LeftMenu_BUArea extends React.Component {
  constructor(props) {
    super(props);
  ...
  loadData=()=>{
    ...
    handleClick=(item)=>{
      ...
      if(menuNodeObj!=false){menuNodeObj.businessClassIds=menuNodeObj.nodeId};
  ...
  render(){
    const loop = data => data.map((item) => {
        ...
        if (item.children!==null && item.children!==undefined) {
          ...
          let tmpIcon="profile";
          let title=<span style={{fontSize:'14px'}} >{item.label}</span>;
          if(item.parentNodeId=='root'){
            title=<span style={{fontSize:'14px'}} ><Icon type={tmpIcon} style={{fontSize:'14px'}} /><span>{item.label}</span></span>;
          }
...
export default LeftMenu_BUArea;
