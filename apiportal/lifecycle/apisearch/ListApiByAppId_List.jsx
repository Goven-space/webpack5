...
class ListApiByAppId_List extends React.Component {
  ...
  loadData=(pagination=this.state.pagination,filters={},sorter={})=>{
    ...
    if(value!==''){
      searchFilters={"mapUrl":value,"creator":value,"creatorName":value,configName:value,tags:value,"editTime":value};
    }
    ...
    return (
        <div style={{ background: '#fff',padding:25,borderRadius:'4px'}} >
          <Modal key={Math.random()} title='申请API的调用权限' maskClosable={false}
            ...
            <NewAppyApi  apiIds={[this.state.currentRecord.id]} apiNames={[this.state.currentRecord.configName]} close={this.closeModal} />
          </Modal>
           <Table
             bordered={false}
