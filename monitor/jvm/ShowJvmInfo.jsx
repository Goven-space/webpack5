...
const jvminfoUrl=URI.LIST_MONITOR.jvminfo; // 内存线程信息
const systemgcUrl=URI.LIST_MONITOR.systemgc; // 内存回收命令
...
class ShowJvmInfo extends React.Component{
  ...
  //回收内存
  systemgc=()=>{
    AjaxUtils.get(systemgcUrl,(data)=>{
        if(data.state===false){
          AjaxUtils.showError(data.msg);
        }else{
    ...
    return (
      <Spin spinning={this.state.mask} tip="Loading..." >
        <div style={{paddingBottom:'20px'}} className='apidoc'  >
          <Button  type="primary" onClick={this.refresh} icon="reload" style={{display:'none'}}  >刷新</Button>{' '}
            <Tabs>
          ...
          <TabPane  tab="内存使用情况" key="meminfo"  >
              <div style={{float:'right',margin:'5px'}} ><Button  type="primary" onClick={this.systemgc} icon="sync"  >回收内存</Button></div>
              <h2>内存统计信息</h2>
                    <table style={{width:'100%'}}><tbody>
                      <tr key='in5'>
                      <th style={{width:'30%'}} >数据名称</th>
