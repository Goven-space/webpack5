...
import NewNetworkMonitor from '../form/NewNetworkMonitor';
...
const LIST_URL=URI.LIST_MONITOR_NETWORK.list;
const DELETE_URL=URI.LIST_MONITOR_NETWORK.delete;
const exporConfigUrl=URI.LIST_MONITOR_NETWORK.export;
const START_URL=URI.LIST_MONITOR_NETWORK.start;
const CLOSE_URL=URI.LIST_MONITOR_NETWORK.close;

class ListNetworkMonitor extends React.Component {
  constructor(props) {
    super(props);
  ...
  addTabPane=(id,name,record)=>{
      ...
      let content;
      if(id==='New'){
        content=(<NewNetworkMonitor id="" appId={this.props.appId}  close={this.closeCurrentTab} />);
      }else if(id==='Edit'){
        content=(<NewNetworkMonitor id={record.id} appId={this.props.appId}  close={this.closeCurrentTab} />);
      }
      ...
      width:'15%',
        ...
        render: (text,record) => {
                return (<span>
                  ...
                  <a onClick={AjaxUtils.showConfirm.bind(this,'启动监听','确定要启动网络连通性监听?',this.start.bind(this,record.id))} >启动</a>
                  <Divider type="vertical" />
                  <a onClick={AjaxUtils.showConfirm.bind(this,'关闭监听器','确定要关闭监听吗?',this.close.bind(this,record.id))} >关闭</a>
                </span>);
        }
    }];
    ...
    return (
      <span>
            <Tabs
              ...
              hideAdd={true}
            >
            <TabPane tab="监听器列表" key="home" style={{padding:'0px'}}>
              <Row style={{marginBottom:5}} gutter={0} >
                <Col span={12} >
                <ButtonGroup>
                  <Button  type="primary" onClick={this.onActionClick.bind(this,'New')} icon="plus"  >新增监听器</Button>
...
export default ListNetworkMonitor;
