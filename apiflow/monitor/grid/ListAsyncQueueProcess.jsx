...
const RUN_URL=URI.ESB.CORE_ESB_MONITOR.runAsyncQueueProcessInstance;

class ListAsyncQueueProcess extends React.Component {
  constructor(props) {
    super(props);
    this.appId=this.props.appId;
    this.applicationId=this.props.applicationId;
    this.runType=this.props.runType||"1";
    this.state={
  ...
  componentWillReceiveProps=(nextProps)=>{
    if(this.runType!==nextProps.runType && nextProps.runType!==undefined){
  ...
  loadData=(pagination=this.state.pagination, filters={}, sorter={})=>{
    let url=LIST_URL+"?runType="+this.runType+"&applicationId="+this.applicationId;
    ...
    const columns=[
      {
        ...
        render: (text,record) => {
          if(text===1){
            ...
            return <Tag color='orange'>队列</Tag>
          }else{
    ...
    return (
      <div>
            <Tabs
              ...
              hideAdd={true}
            >
            <TabPane tab="未结束流程" key="home" style={{padding:'0px'}}>
              <Row style={{marginBottom:5}} gutter={0} >
                <Col span={12} >
                <ButtonGroup>
                  <Button  type="primary" onClick={AjaxUtils.showConfirm.bind(this,"标记为已结束","确定要把流程标记为结束状态吗?如果流程正在运行或睡眠时并不会停止运行!",this.endProcess)}  disabled={!hasSelected} icon="poweroff"   >结束流程</Button>
                  <Button  type="ghost" onClick={AjaxUtils.showConfirm.bind(this,"立运行流程","确定要立即运行选中流程吗?",this.runProcess)}  disabled={!hasSelected} icon="safety"   >立即运行</Button>
...
export default ListAsyncQueueProcess;
