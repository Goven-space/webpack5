...
const MOVE_HISTORY_URL=URI.ESB.PROCESS_HISTORY.movetohistory;
...
class ListStopProcess extends React.Component {
  constructor(props) {
    ...
    this.url=LIST_URL+"?runType=0&applicationId="+this.applicationId+"&processId="+this.processId;
  ...
  //移动到归档流程列表中
  movetohistory=()=>{
    let ids=this.state.selectedRows.map((item)=>{return item.transactionId;}).join(",");
    AjaxUtils.post(MOVE_HISTORY_URL,{transactionIds:ids},(data)=>{
    ...
    const columns=[
      {
        ...
        render: (text,record) => {
          ...
          title: '补偿状态',
          dataIndex: 'compensateResultStatus',
          width:'8%',
          render: (text,record) => {
            if(text===0){
              return <Tag color='gold'>失败</Tag>
            }else if(text===1){
              return <Tag color='#87d068'>成功</Tag>
            }else if(text===2){
              return <Tag>取消</Tag>
    ...
    return (
      <div>
            <Tabs
              ...
              hideAdd={true}
            >
            <TabPane tab="已结束流程" key="home" style={{padding:'0px'}}>
              <Row style={{marginBottom:5}} gutter={0} >
                <Col span={6} >
                <ButtonGroup>
                  <Button  type="ghost" onClick={this.showConfirm} icon="delete"  disabled={!hasSelected} >删除</Button>
                  <Button  type="ghost" onClick={AjaxUtils.showConfirm.bind(this,"归档流程","您确定要归档选中流程吗?",this.movetohistory)} icon="copy"  disabled={!hasSelected} >归档</Button>
