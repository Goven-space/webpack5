...
const getColumnsByTableName=URI.ESB.DBTABLE_READNODE.getTableFields;
const parsesqlColumnsUrl=URI.ESB.DBTABLE_READNODE.parsesqlColumns;
const ruleSelectUrl=URI.ESB.CORE_ESB_RULE.select;
const Option = Select.Option;
const ButtonGroup = Button.Group;

class SqlReadNodeColumns extends React.Component {
  constructor(props) {
    super(props);
  ...
  renderFieldType(index, key, text,record) {
    ...
    if(index!==this.state.curEditIndex){
      if(digits!==undefined && digits!=='undefined' && digits!=='' && digits!==0){
        return text+"["+record.colLength+","+digits+"]";
      }else{
    ...
    return (
      <div>
          <div style={{paddingBottom:10}} >
              ...
              <Button  onClick={AjaxUtils.showConfirm.bind(this,"从SQL导入字段?","如果当前已经有字段配置数据,导入字段会丢失现有数据,必须保证SQL执行正确且有数据返回!",this.loadTableColumnsFromSql.bind(this))} icon="select"  >从SQL中分析</Button>
...
export default SqlReadNodeColumns;
