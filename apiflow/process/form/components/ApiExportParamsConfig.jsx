...
class ApiExportParamsConfig extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentWillReceiveProps=(nextProps)=>{
    if(nextProps.outPutParaTableData !== this.props.outPutParaTableData) {
      this.getDataByJsonExport(nextProps.outPutParaTableData)
    }
  }

  getDataByJsonExport = (valueArr) => {
    this.setState({
      data: valueArr
    })
  }
  ...
  render() {
    ...
    const columns=[{
      title: '参数名',
      dataIndex: 'paramsId',
      render: (text, record, index) => this.renderEditText(index,'paramsId', text,"支持中文名,必须全局唯一"),
      width:'25%',
    },{
      title: '参数值',
      dataIndex: 'paramsValue',
      render: (text, record, index) =>this.renderEditText(index,'paramsValue',text,"使用JsonPath取值并作为本节点的输出参数标识如:$.data.userid"),
      width:'65%',
    },{
      title: '操作',
      dataIndex: 'action',
    ...
    return (
      <div>
        <Modal  title='从API响应结果中自动分析' maskClosable={false}
            width='1000px'
            style={{ top: 20 }}
            ...
            请把API的结果JSON贴入后点击分析按扭
        </Modal>
        <Button  onClick={this.insertRow}  type="primary"   icon='plus'  style={{marginBottom:'5px'}} >新增输出参数</Button>{' '}
        <Button  onClick={this.showLoadFromJsonModal}  type="ghost"   icon='arror-up'  style={{marginBottom:'5px'}} >从API结果JSON中自动分析</Button>
        <Table
...
export default ApiExportParamsConfig;
