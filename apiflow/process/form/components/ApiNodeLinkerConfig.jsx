...
import { Table,Button,Tag, Icon, Drawer, Modal, Input, Row , Col, Popconfirm, Card } from 'antd';
...
class ApiNodeParamsConfig extends React.Component {
  ...
  getApiData = (apiSelectRows=[]) => {
    if(!apiSelectRows.length) {
      ...
      if(paramsDocs) {
        let newParamsDocs = typeof paramsDocs === 'string'? JSON.parse(paramsDocs):paramsDocs
         newParamsDocs = newParamsDocs.map(item => {
          ...
          if(item.children && item.children.length) {
            obj.children = this.dealData(item.children,'sampleValue','fieldName','fieldType')
          }
          return obj
        })
        this.setState({
  ...
  dealData = (arr=[],paramsValue='',paramsName, fieldType) => {
    return arr.map(item => {
      ...
      if(item.children && item.children.length) {
        obj.children = this.dealData(item.children,paramsValue,paramsName,fieldType)
      }
      return obj
    })
  }
	...
	getMasterApiData = (arr=[]) => {
	 ...
	 const newParamsDocs = arr.map(item => {
		...
		if(item.children && item.children.length) {
			obj.children = this.dealData(item.children,'fieldValue','fieldName','fieldType')
		}
		return obj
	})
  ...
  renderEditText(index, key, text,placeholder, record) {
    ...
    return (<EditText value={text} size='default' placeholder={placeholder} onChange={value => this.handleChange(key, index, value, record)} options={{style:{width:'70%'}}}/>);
  }
  ...
  renderParamsValue(index, key, text,placeholder, record) {
    ...
    return (<AjaxEditSelect value={text} url={url}  onChange={value => this.handleChange(key, index, value, record)} />);
  }
  ...
  renderProcessRules(index, key, text, record) {
    ...
    return (
      ...
      value={text}
      size='small'
      onChange={(value,label) => this.handleChange(key, index, value, record, label)}
      />);
  }
  ...
  addSubRow = (record) => {
    ...
    record.children.push({id:key,paramsName:'',paramsValue:'',paramsSource:'AUTO',paramsId:''});
  }
  ...
  insertRow = (id, data = this.state.data) => {
    for (var index in data) {
      if (data[index].id == id) {
        let newRow = { id: AjaxUtils.guid(), paramsName:'',paramsValue:'',paramsSource:'AUTO',paramsId:'' };
  ...
  closeModal = (newRowData) => {
    if (newRowData === false || !newRowData.id) { return; }
  ...
  render() {
    const { data, action, masterDataModalVisible, rowsData, pagination } = this.state;
    ...
    const columns=[{
      ...
      render: (text, record, index) => {
          return this.renderParamsValue(index,'defaultValue', text, undefined, record)
        },
      width:'18%',
    } ,{
      title: '参数类型',
      ...
      title: '绑定计算规则',
      dataIndex: 'fieldRule',
      render: (text, record, index) =>this.renderProcessRules(index,'fieldRule',text, record),
      width:'20%',
    }];
    const masterColumns = [{
      title: '主数据名称',
    ...
    let content
    if (action == 'more') {
      content = <EditServiceMoreParams currnetEditRow={this.state.currentRecord} configId={this.state.configId} close={this.closeModal} />
    }
