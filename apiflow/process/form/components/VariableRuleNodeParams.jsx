...
class VariableRuleNodeParams extends React.Component {
  ...
  render() {
    ...
    let columns=[{
      ...
      dataIndex: 'colValue',
      width:'20%',
      render: (text, record, index) => this.renderEditTextArea(index,'colValue', text,record,"$.变量,$.http.变量"),
    },{
      title: '类型',
      dataIndex: 'dataType',
      width:'10%',
      render: (text, record, index) => this.renderDataType(index,'dataType', text,record),
    },{
      title: '绑定数据转换或生成规则',
      dataIndex: 'ruleId',
      width:'30%',
