...
import NewHttpConfig from '../form/NewHttpConfig';
...
class ListHttpConfig extends React.Component {
  constructor(props) {
    super(props);
  ...
  onActionClick=(action,record,url)=>{
    if(action==="Delete"){
      ...
      this.addTabPane('New','新增HTTP代理配置');
    }
  }
  ...
  addTabPane=(id,name,record)=>{
      ...
      let content;
      if(id==='New'){
        content=(<NewHttpConfig id="" appId={this.props.appId}  close={this.closeCurrentTab} />);
      }else if(id==='Edit'){
        content=(<NewHttpConfig id={record.id} appId={this.props.appId}  close={this.closeCurrentTab} />);
      }
  ...
  render(){
    ...
    const expandedRow=(record)=>{
        return (<ApiQpsAndLogMonitor id={record.id} />);
    }
    ...
    return (
      <span>
            <Tabs
              ...
              hideAdd={true}
            >
            <TabPane tab="HTTP代理配置" key="home" style={{padding:'0px'}}>
              <Row style={{marginBottom:5}} gutter={0} >
                <Col span={12} >
                <ButtonGroup>
                  <Button  type="primary" onClick={this.onActionClick.bind(this,'New')} icon="plus"  >新增HTTP代理</Button>
