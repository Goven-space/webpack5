...
import NewDataEncryptionRule from '../form/NewDataEncryptionRule';
...
class ListDataEncryptionRule extends React.Component {
  constructor(props) {
    super(props);
  ...
  render(){
    ...
    const columns=[{
        ...
        dataIndex: 'direction',
        width: '15%',
        render: (text,record) =>{if(text==='IN'){return '前端请求数据';}else{return "后端返回数据";}}
      },{
        title: '类型',
        dataIndex: 'action',
        width: '15%',
        render: (text,record) =>{if(text==='encry'){return <Tag color='green'>加密</Tag>;}else{return <Tag color='blue'>解密</Tag>;}}
      },{
        title: '服务器',
    ...
    return (
      <div >
          <Modal key={Math.random()}  maskClosable={false}
            ...
            <NewDataEncryptionRule  id={currentId}  close={this.closeModal} />
          </Modal>

        <Row style={{marginBottom:5}} gutter={0} >
          <Col span={12} >
            <ButtonGroup>
            <Button  type="primary" onClick={this.onActionClick.bind(this,'New')} icon="file-add" >新增加解密规则</Button>
...
export default ListDataEncryptionRule;
