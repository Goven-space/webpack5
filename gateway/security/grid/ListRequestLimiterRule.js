...
import NewRequestLimiterRule from '../form/NewRequestLimiterRule';
...
class ListRequestLimiterRule extends React.Component {
  constructor(props) {
    super(props);
  ...
  render(){
    ...
    const columns=[{
        ...
        dataIndex: 'qpsUserId',
        width: '15%',
        render: (text,record) =>{if(text===''||text===undefined){return '所有用户';}else{return text;}}
      },{
        title: '服务器',
    ...
    return (
      <div >
          <Modal key={Math.random()}  maskClosable={false}
            ...
            <NewRequestLimiterRule  id={currentId}  close={this.closeModal} />
          </Modal>

        <Row style={{marginBottom:5}} gutter={0} >
          <Col span={12} >
            <ButtonGroup>
            <Button  type="primary" onClick={this.onActionClick.bind(this,'New')} icon="file-add" >新增限速规则</Button>
...
export default ListRequestLimiterRule;
