...
import NewServies from '../form/NewServies';

//网关Services管理

const Search = Input.Search;
const confirm = Modal.confirm;
const LIST_URL=URI.CORE_GATEWAY_SERVICES.list;
const DELETE_URL=URI.CORE_GATEWAY_SERVICES.delete;
const exportServices=URI.CORE_GATEWAY_SERVICES.exportServicesConfig;
...
class ListServices extends React.Component {
  ...
  showConfirm=()=>{
      var self=this;
      confirm({
      title: '您确认要删除选中Services吗,如果已经有API引用则会造成API不能访问?',
      content: '注意:删除后不可恢复!',
      onOk(){
  ...
  search=(value)=>{
    ...
    let sorter={};
    let searchFilters={};
    searchFilters={"name":value,"host":value};
  ...
  render(){
    ...
    const columns=[{
        ...
        dataIndex: 'status',
        width:'6%',
        render: (text,record) => {if(text===1){return <Tag color='green'>启用</Tag>} else if(text===0){return <Tag color='red'>停用</Tag>}}
      },{
        title: '操作',
        dataIndex: '',
        key: 'x',
    ...
    return (
      <div>
          <Modal key={Math.random()} title="后端服务属性" maskClosable={false}
            ...
            <NewServies  id={currentId} close={this.closeModal} />
          </Modal>
        <Row style={{marginBottom:5}} gutter={0} >
          <Col span={12} >
            <Button  type="primary" onClick={this.onActionClick.bind(this,'New')} icon="plus" >新建后端服务</Button>{' '}
