...
class form extends React.Component{
  constructor(props){
    ...
    this.beanType=this.interface.substring(this.interface.lastIndexOf(".")+1,this.interface.length); //根据接口类型自动计算beanType
    this.state={
      mask:true,
      formData:{}
    };
  ...
  componentDidMount(){
    ...
    if(id===undefined){
        FormUtils.getSerialNumber(this.props.form,"beanId",this.appId,"PLUGIN");
  ...
  onSubmit = () => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
          ...
          postData.classPath="cn.restcloud.userapp.gateway."+postData.beanId;
