...
class ListTestCase extends React.Component {
  constructor(props) {
    super(props);
    this.appId='';
  ...
  render(){
    ...
    const columns=[{
      ...
      render:text => {
        ...
        render: (text,record) => {
          if(text==='无'){
            return (<a onClick={this.onActionClick.bind(this,"Task",record)}>加入</a>);
          }else{
            return <Tag title={text}>已安排</Tag>;
          }
        }
      },{
        title: '操作',
        ...
        width:'7%',
        render: (text,record) => {
          return (<div><Popconfirm title="确定要复制我吗?" onConfirm={this.onActionClick.bind(this,"Copy",record)}><a>复制</a></Popconfirm></div>);
        }
      },];
    ...
    return (
      <Tabs
        ...
        hideAdd={true}
      >
      <TabPane tab="测试用例列表" key="home" style={{padding:'0px'}}>
        <Modal key={Math.random()} title="加入测试任务" maskClosable={false}
         ...
         <div style={divStyle}>
          <ButtonGroup>
          <Button  type="primary" onClick={this.onActionClick.bind(this,'New')} icon="plus"  >新增测试</Button>
