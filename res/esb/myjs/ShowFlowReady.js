var initColor = 'url(../images/nodebg-gray-min.png)';
var animateColor = 'url(../images/nodebg-red-min.png)';
var animateAfterColor = 'url(../images/nodebg-green-min.png)';
	...
	for(var i=0;i<insNodeList.length;i++){
		...
		if(nodeType!=='router'){
			...
			if(nodeObj.currentStatus==='current'){
					...
					processNodeObj.css('background-image', "url(../images/nodebg-red-min.png)");
					...
					processNodeObj.css('background-image', "url(../images/nodebg-green-min.png)");
			}else{
					processNodeObj.css('background-color', '#f4f4f4');
					processNodeObj.css('background-image', "url(../images/nodebg-gray-min.png)");
			}
		}
		...
		if(msg!=="" && msg!==undefined || nodeObj.pNodeType==='router'){
				if(nodeObj.pNodeType==='router'){
					...
					if(routerObj!=undefined){
						console.log(nodeObj.currentStatus+"=>"+nodeObj.sourceId+"=>"+ nodeObj.targetId);
						if(nodeObj.currentStatus=='end'){
							routerObj.setPaintStyle({fill:'#339933',stroke:'#339933',strokeWidth:2});
						}
					}
				}else{
					//是普通节点的情况下
						...
						if(nodeMsgObj.length>0){
							...
							var leftX=offset.left/GoalTransform+left*GoalTransform;
	...
	var timer = setInterval(function() {
		...
		if (j < gradientColorArr.length) {
				...
				// console.log(nodePath[index]);
				 ...
				 var targetId=nodeIdArray[1];
					...
					if(connsObj!==undefined){
						connsObj.setPaintStyle({fill:'#ff0000',stroke:'#ff0000',strokeWidth:2});
					}
			}else{
			...
			if(connsObj!==null && connsObj!=undefined){
				connsObj.setPaintStyle({fill:'#1e8151',stroke:'#ccc',strokeWidth:2});
			}
