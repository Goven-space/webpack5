...
layui.use('element', function(){
  ...
  var url=parent.host+"/rest/etl/process/node/template/list/select";
  $.ajax({
...
function createNode(data){
  ...
  for(var i=0;i<data.length;i++){
    ...
    for(var x=0;x<subNode.length;x++){
      ...
      var tip=subNode[x].tip;
      let nodeIcon="node.png";
      if(nodeType=='start'){nodeIcon='start.png';}
      if(nodeType=='end'){nodeIcon='end.png';}
      subNodeHtml+='<div class="controler" title=\""+tip+"\" onmousedown="startNode(\''+nodeType+'\',\''+templateId+'\',\''+nodeName+'\',\''+className+'\')"><img src="../images/'+nodeIcon+'" height="16px" > '+nodeName+'</div>';
    }
