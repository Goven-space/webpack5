...
class form extends React.Component{
  ...
  loadFormData=()=>{
    ...
    if(id===undefined || id===''){
        ...
        FormUtils.getSerialNumber(this.props.form,"nodeId",this.appId,"BS");
    }else{
        ...
        <Tabs size="large">
          <TabPane  tab="业务域属性" key="props">
            <FormItem key='parentNodeId'  label="上级业务域" {...formItemLayout4_16} hasFeedback>
            {
              ...
              (<TreeNodeSelect url={`${treeUrl}?nodeId=${this.props.nodeId?this.props.nodeId:''}`} options={{dropdownStyle:{maxHeight: 400, overflow: 'auto' }}}/>)
            }
            </FormItem>
            <FormItem
              label="业务域名称"
          ...
          {/* <TabPane  tab="菜单事件" key="Hystrix"  >
            <Row>
              <Col span={4}></Col>
              <Col span={16}>
                <div style={{border:'1px #cccccc solid',minHeight:'150px',margin:'2px',borderRadius:'0px'}}>
                  <CodeMirror ref='codeMirror'
                  value={this.state.formData.eventCode}
                  onChange={this.updateCode}
                  options={{lineNumbers: true,mode: 'javascript',autoMatchParens:true}}
                  />
              </div>
              可以使用RdbUtil,RdbMapperUtil,MongoUtil等工具类来执行SQL动态修改菜单的参数或菜单所包含的数据量,使用returnDoc.put("count",12)返回数量
              </Col>
            </Row>
          </TabPane> */}
        </Tabs>
