...
const DataImportUrl=URI.CORE_MasterData.datamappingImport;

class SelectListMasterData extends React.Component {
  constructor(props) {
    super(props);
  ...
  selectOk=()=>{
    ...
    let ids=this.state.selectedRowKeys.join(",");
    AjaxUtils.post(DataImportUrl,{masterId:ids,mappingCategoryId:this.categoryId},(data)=>{
      this.setState({loading:false})
      if(data.state==false){
        AjaxUtils.showError(data.msg);
      }else{
        AjaxUtils.showInfo("成功选择一个主数据");
  ...
  render(){
    ...
    return (
      <div>
        ...
        <Button  type="primary" onClick={this.selectOk}  >确定选择</Button>{' '}
        <Button  type="ghost" onClick={this.closeModal}  >关闭</Button>{' '}
        </div>
    </div>
    );
  }
...
export default SelectListMasterData;
