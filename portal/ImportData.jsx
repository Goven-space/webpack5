...
const uploadUrl=URI.CORE_FILE.uploadResource_mgdb; //改成mongodb
const deleteUrl=URI.CORE_FILE.deleteFile; //无需改变
const downloadUrl=URI.CORE_FILE.download_mgdb; //改成mongodb
...
class form extends React.Component{
  ...
  onSubmit = () => {
    ...
    if(fileName.indexOf(".bson")===-1 && fileName.indexOf(".json")===-1){
      AjaxUtils.showError("只能上传bson或json格式的文件!");
      return;
    }
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
          ...
          AjaxUtils.post(saveDataUrl,postData,(data)=>{
              this.setState({mask:false,fileList:[]});
              if(data.state===false){
  ...
  onFileChange=(info)=>{
          ...
          fileList = fileList.map((file) => {
            if (file.response) {
              ...
              file.url=downloadUrl.replace("{id}",file.uid); //改成mongodb
              file.parentDocId='0';
            }
            return file;
          });
    ...
    return (
      ...
      <Form style={{marginRight:'20px'}}>
	     <FormItem  label="数据包"  {...formItemLayout4_16}>
          <Upload {...uploadProps}>
            <Button>
              <Icon type="upload" /> 点击上传设计打包文件(*.bson,*.json格式)
            </Button>
          </Upload>
        </FormItem>
        <FormItem
